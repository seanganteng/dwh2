...
$skenario = [
    // Sama dengan query "Sales by month for chart" (mode tanpa filter tahun/bulan) di index.php baris ~216-220
    'penjualan_bulanan' => [
        'label' => 'Penjualan/Bulan-Tahun',
    ...
    // Sama dengan query "Top 10 by category" (mode tanpa filter) di index.php baris ~122-137
    'top_kategori' => [
        'label' => 'Top Kategori (Qty)',
        'dwh_tables' => 3, 'dwh_joins' => 2,
        'oltp_tables' => 2, 'oltp_joins' => 1,
    ...
    // Sama dengan query "$sellerQuery" (mode tanpa filter) di index.php baris ~389-406, diberi ORDER BY/LIMIT
    // untuk meniru pengurutan top-10-by-revenue yang aslinya dilakukan via usort() di PHP
    'top_seller' => [
        ...
        'oltp_sql' => "SELECT s.seller_id AS label,\n  COUNT(*) AS total_qty,\n  SUM(oi.price) AS total_revenue\nFROM order_items oi\nJOIN sellers s\n  ON oi.seller_id = s.seller_id\nGROUP BY s.seller_id\nORDER BY total_revenue DESC\nLIMIT 10;",
    ],
    // Sama dengan query "$customerQuery" (mode tanpa filter) di index.php baris ~430-447
    'top_pelanggan' => [
        'label' => 'Top Pelanggan (Revenue)',
        'dwh_tables' => 3, 'dwh_joins' => 2,
        'oltp_tables' => 3, 'oltp_joins' => 2,
        'dwh_sql' => "SELECT COALESCE(dp.pelanggan_id, CAST(fp.pelanggan_key AS varchar)) AS label,\n  SUM(fp.total_harga) AS total_revenue\nFROM fakta_penjualan fp\nLEFT JOIN dim_pelanggan dp\n  ON fp.pelanggan_key = dp.pelanggan_key\nJOIN dim_waktu w\n  ON fp.waktu_id = w.waktu_id\nGROUP BY COALESCE(dp.pelanggan_id, CAST(fp.pelanggan_key AS varchar))\nORDER BY total_revenue DESC\nLIMIT 10;",
        'oltp_sql' => "SELECT c.customer_id AS label,\n  SUM(oi.price) AS total_revenue\nFROM customers c\nJOIN orders o\n  ON c.customer_id = o.customer_id\nJOIN order_items oi\n  ON o.order_id = oi.order_id\nGROUP BY c.customer_id\nORDER BY total_revenue DESC\nLIMIT 10;",
    ],
    // Sama dengan query "$ratingYearQuery" (mode tanpa filter) di index.php baris ~340-361
    'rating_tahunan' => [
        'label' => 'Rating/Tahun',
        'dwh_tables' => 2, 'dwh_joins' => 1,
        'oltp_tables' => 2, 'oltp_joins' => 1,
        'dwh_sql' => "SELECT dw.tahun AS label,\n  ROUND(AVG(fr.skor_review), 2) AS avg_rating\nFROM fakta_review fr\nJOIN dim_waktu dw\n  ON fr.waktu_id = dw.waktu_id\nGROUP BY dw.tahun\nORDER BY dw.tahun;",
